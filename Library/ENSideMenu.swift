...
open class ENSideMenu : NSObject, UIGestureRecognizerDelegate {
    ...
    open var shouldRotate: Bool = false
    fileprivate var isOrientationChanging: Bool = false
    fileprivate var lastOrientation: Int!
    fileprivate var darkenBackgroundView: UIView?
    ...
    public init(sourceView: UIView, menuPosition: ENSideMenuPosition, blurStyle: UIBlurEffectStyle = .light) {
        ...
        lastOrientation = (UIDevice.current.orientation.rawValue == 4) ? 3 : UIDevice.current.orientation.rawValue
        
        NotificationCenter.default.addObserver(self, selector: #selector(ENSideMenu.deviceOrientationChanged(_:)), name: NSNotification.Name.UIDeviceOrientationDidChange, object: nil)
    ...
    fileprivate func adjustFrameDimensions( _ width: CGFloat, height: CGFloat ) -> (CGFloat,CGFloat) {
        if floor(NSFoundationVersionNumber) <= NSFoundationVersionNumber_iOS_7_1 && (UIApplication.shared.statusBarOrientation == UIInterfaceOrientation.landscapeRight ||
            ...
            if isOrientationChanging && floor(NSFoundationVersionNumber) > NSFoundationVersionNumber_iOS_8_4 {
    ...
    func showDarkenBackgroundView() {
        ...
        if self.darkenBackgroundView == nil {
            ...
            self.darkenBackgroundView?.backgroundColor = UIColor.black
            self.darkenBackgroundView?.alpha = 0
            
            let tap = UITapGestureRecognizer(target:  self, action: #selector(ENSideMenu.toggleMenu as (ENSideMenu) -> () -> ()))
